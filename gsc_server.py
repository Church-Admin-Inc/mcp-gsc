...
class _NoLoginOAuthProvider(OAuthAuthorizationServerProvider[AuthorizationCode, RefreshToken, AccessToken]):
    """Auto-approving OAuth 2.1 authorization server for the SSE transport.

    MCP clients (e.g. claude.ai custom connectors) require a working OAuth
    handshake — metadata discovery, dynamic client registration, and a token
    exchange — before they'll connect at all, even when the server doesn't
    need per-caller identity. Real authorization to Google Search Console is
    already enforced server-side via GSC_CREDENTIALS_PATH/client_secrets.json
    (see get_gsc_service()), so this provider just rubber-stamps the MCP-level
    handshake rather than gating on a second, redundant login.
    """
    ...
    async def exchange_refresh_token(
        ...
        return self._issue_token(client.client_id, scopes or refresh_token.scopes, None)
    ...
    def _issue_token(self, client_id: str, scopes: List[str], resource: Optional[str]) -> OAuthToken:
...
def _build_fastmcp() -> FastMCP:
    """Build the FastMCP instance, adding an OAuth shim for SSE deployments.

    Remote MCP clients require a functioning OAuth handshake to connect over
    SSE (see MCP_PUBLIC_URL below); stdio clients (Claude Desktop, Claude
    Code) never hit this path and are unaffected.
    """
    if mcp_transport in {"sse", "http"}:
        ...
        if public_url:
            ...
            return FastMCP(
                ...
                host=mcp_host,
                port=mcp_port,
                auth_server_provider=_NoLoginOAuthProvider(),
                auth=auth_settings,
            )
    return FastMCP("gsc-server", host=mcp_host, port=mcp_port)
